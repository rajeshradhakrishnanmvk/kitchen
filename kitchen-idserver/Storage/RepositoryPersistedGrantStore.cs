...
namespace kitchen_idserver.Storage
{
    public class RepositoryPersistedGrantStore : IPersistedGrantStore
    {
        ...
        public async Task<IEnumerable<PersistedGrant>> GetAllAsync(PersistedGrantFilter filter)
            => await Task.FromResult(_repository.Where<PersistedGrant>(i => i.SubjectId == filter.SubjectId
            && i.Type == filter.Type
            && i.SessionId == filter.SessionId
            && i.ClientId == filter.ClientId).AsEnumerable());

        public async Task RemoveAllAsync(PersistedGrantFilter filter)
        {
            _repository.Delete<PersistedGrant>(i => i.SubjectId == filter.SubjectId
           && i.SessionId == filter.SessionId && i.ClientId == filter.ClientId && i.Type == filter.Type);
        }
    }
}